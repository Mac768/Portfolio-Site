...
// 1. Loop through all your project wrapper elements
// Note: Replace '.project-card' with whatever class wrapper container you used for each case study
document.querySelectorAll('.project-card').forEach(card => {
  card.addEventListener('click', (e) => {
    // If your project cards use <a> tags, prevent them from redirecting or jumping
    e.preventDefault();

    // Grab the specific text content inside the clicked card
    const titleText = card.querySelector('h2').innerText;
    const descriptionText = card.querySelector('p').innerText;

    // Inject the text into the modal template
...
// 3. Close the modal if clicking anywhere outside the content box
